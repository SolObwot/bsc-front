...
export const fetchDepartmentObjectives = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response =
        await performanceMeasureService.getDepartmentObjectives();
  ...
  },
  {
    condition: (_, { getState }) => {
      const { performanceMeasure } = getState();
      return !performanceMeasure.loading.department;
    },
  }
);
...
export const fetchPerformanceMeasures = createAsyncThunk(
  ...
  async (params = { page: 1 }, { rejectWithValue }) => {
    try {
      const response = await performanceMeasureService.getPerformanceMeasures(
        params
      );
      return response;
    } catch (error) {
  ...
  async (params, { rejectWithValue }) => {
    try {
      const firstPageResponse =
        await performanceMeasureService.getPerformanceMeasures({
          ...params,
          page: 1,
        });
      ...
      if (totalPages > 1) {
        ...
        for (let page = 2; page <= totalPages; page++) {
          pagePromises.push(
            performanceMeasureService.getPerformanceMeasures({
              ...params,
              page,
            })
          );
...
export const fetchPerformanceMeasureById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = await performanceMeasureService.getPerformanceMeasure(
        id
      );
      return response;
    } catch (error) {
...
// Fetch a specific agreement by ID
export const fetchAgreementById = createAsyncThunk(
  "performanceMeasure/fetchAgreementById",
  async (id, { rejectWithValue }) => {
    try {
      const response = await performanceMeasureService.getAgreementById(id);
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data || { message: error.message }
  ...
  },
  {
    condition: (_, { getState }) => {
      const { performanceMeasure } = getState();
      return !performanceMeasure.loading.selectedAgreement;
    },
  }
);
...
export const createPerformanceMeasure = createAsyncThunk(
  ...
  async (measureData, { rejectWithValue }) => {
    try {
      const response = await performanceMeasureService.createPerformanceMeasure(
        measureData
      );
      return response;
    } catch (error) {
...
export const updatePerformanceMeasure = createAsyncThunk(
  ...
  async ({ id, data }, { rejectWithValue }) => {
    try {
      const response = await performanceMeasureService.updatePerformanceMeasure(
        id,
        data
      );
      return response;
...
export const fetchAllDashboardPerformanceMeasures = createAsyncThunk(
  ...
  async ({ agreement_id }, { rejectWithValue }) => {
    if (!agreement_id) {
      return rejectWithValue({
        message: "The agreement_id parameter is required.",
      });
    }

    try {
      const firstPageResponse =
        await performanceMeasureService.getDashboardPerformanceMeasures({
          agreement_id,
          page: 1,
        });
      const totalPages = firstPageResponse.last_page;
      ...
      if (totalPages > 1) {
        ...
        for (let page = 2; page <= totalPages; page++) {
          pagePromises.push(
            performanceMeasureService.getDashboardPerformanceMeasures({
              agreement_id,
              page,
            })
          );
  ...
  {
    condition: ({ agreement_id }, { getState }) => {
      const { performanceMeasure } = getState();
      if (!agreement_id) return false;
      return !performanceMeasure.loading.allDashboardMeasures;
    },
  }
);
...
export const fetchAllPerformanceMeasuresByAgreement = createAsyncThunk(
  ...
  async ({ agreement_id, ...params }, { rejectWithValue }) => {
    ...
    try {
      const firstPageResponse =
        await performanceMeasureService.getPerformanceMeasuresByAgreement(
          agreement_id,
          { ...params, page: 1 }
        );
      const totalPages = firstPageResponse.last_page;
...
const initialState = {
  ...
  perspectives: [],
  agreement: null,
  selectedAgreement: null, // To store a single fetched agreement
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchDepartmentObjectives.rejected, (state, action) => {
      ...
      state.error.department = action.payload || {
        message: "Failed to fetch department objectives",
      };
    });
    ...
    builder.addCase(fetchPerformanceMeasures.rejected, (state, action) => {
      state.loading.measures = false;
      state.error.measures = action.payload || {
        message: "Failed to fetch performance measures",
      };
    });
    ...
    builder.addCase(fetchAllPerformanceMeasures.rejected, (state, action) => {
      state.loading.allMeasures = false;
      state.error.allMeasures = action.payload || {
        message: "Failed to fetch all performance measures",
      };
    });
    ...
    builder.addCase(fetchPerformanceMeasureById.rejected, (state, action) => {
      ...
      state.error.fetchSingle = action.payload || {
        message: "Failed to fetch performance measure",
      };
    });

    // Fetch a single agreement
    builder.addCase(fetchAgreementById.pending, (state) => {
      state.loading.selectedAgreement = true;
      state.error.selectedAgreement = null;
    });
    builder.addCase(fetchAgreementById.fulfilled, (state, action) => {
      state.loading.selectedAgreement = false;
      state.selectedAgreement = action.payload;
    });
    builder.addCase(fetchAgreementById.rejected, (state, action) => {
      state.loading.selectedAgreement = false;
      state.error.selectedAgreement = action.payload || {
    ...
    builder.addCase(createPerformanceMeasure.rejected, (state, action) => {
      ...
      state.error.create = action.payload || {
        message: "Failed to create performance measure",
      };
    });
    ...
    builder.addCase(updatePerformanceMeasure.fulfilled, (state, action) => {
      ...
      const index = state.measures.data.findIndex(
    ...
    builder.addCase(updatePerformanceMeasure.rejected, (state, action) => {
      ...
      state.error.update = action.payload || {
        message: "Failed to update performance measure",
      };
    });
    ...
    builder.addCase(deletePerformanceMeasure.fulfilled, (state, action) => {
      ...
      state.measures.data = state.measures.data.filter(
    ...
    builder.addCase(deletePerformanceMeasure.rejected, (state, action) => {
      ...
      state.error.delete = action.payload || {
        message: "Failed to delete performance measure",
      };
    });
    ...
    builder.addCase(
      fetchAllDashboardPerformanceMeasures.fulfilled,
      (state, action) => {
    ...
    builder.addCase(
      fetchAllDashboardPerformanceMeasures.rejected,
      (state, action) => {
        state.loading.allDashboardMeasures = false;
        state.error.allDashboardMeasures = action.payload || {
          message: "Failed to fetch all dashboard performance measures",
        };
      }
    );

    builder.addCase(
      fetchAllPerformanceMeasuresByAgreement.fulfilled,
      (state, action) => {
        state.loading.allDashboardMeasures = false;
        state.dashboardPerformanceMeasures = action.payload;
