...
const VillageModal = ({ isOpen, closeModal, onSubmit, initialData = null }) => {
                ...
                <div className="bg-teal-50 px-6 py-5 border-b border-teal-100">
                  <div className="flex items-center justify-between">
                    <Dialog.Title className="text-lg font-semibold text-gray-800 capitalize">
                      {isEditing ? "Edit Village" : "Create New Village"}
                    </Dialog.Title>
                ...
                <div className="px-6 py-6">
                  {!isEditing && (
                    <div className="mb-4 p-3 bg-blue-50 rounded-md border border-blue-100 flex items-start">
                      ...
                      <p className="text-sm text-blue-700">Create a new village. Select district → county → sub-county → parish to narrow options.</p>
                    </div>
                  )}

                  <VillageForm initialData={initialData} onSubmit={onSubmit} onCancel={closeModal} isModal={true} isEditing={isEditing} />
                </div>
              </Dialog.Panel>
            </Transition.Child>
          </div>
