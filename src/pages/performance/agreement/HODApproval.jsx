...
import { fetchAllDashboardPerformanceMeasures } from "../../../redux/performanceMeasureSlice";
...
const HODApproval = () => {
  ...
  const {
    departmentAgreements: agreements,
    loading,
    error,
    userDepartmentId,
  } = useSelector((state) => state.agreements);
  const { dashboardPerformanceMeasures, loading: measuresLoading } =
  ...
  // Fetch all dashboard measures for HOD approval (adjust agreement_id if needed for specific agreements)
  useEffect(() => {
    dispatch(fetchAllDashboardPerformanceMeasures({ agreement_id: null })); // Or pass specific IDs if available
  }, [dispatch]);
    ...
    return agreements
      ...
      .filter((agreement) => {
        const matchesText =
          ...
          (agreement.creator?.job_title?.name || "")
            .toLowerCase()
            .includes(filterText.toLowerCase());
        ...
        const matchesYear =
          ...
          (() => {
            ...
            const agreementYear = new Date(
              agreement.submitted_at || agreement.created_at
            )
              .getFullYear()
              .toString();
        ...
        const matchesPeriod =
          filterPeriod === "" || agreement.period === filterPeriod;
  ...
  const departments = useMemo(() => {
    ...
    agreements.forEach((a) => {
      if (
        a.department &&
        a.department.id &&
        !departmentMap.has(a.department.id)
      ) {
  ...
  const branches = useMemo(() => {
    ...
    agreements.forEach((a) => {
      if (
        a.creator &&
        a.creator.unit_or_branch &&
        a.creator.unit_or_branch.id &&
        !branchMap.has(a.creator.unit_or_branch.id)
      ) {
  ...
  const getDepartmentOptions = () => {
    ...
    if (
      userDepartmentId &&
      !options.some((opt) => opt.value === userDepartmentId)
    ) {
      const deptName =
        agreements.find(
          (a) => a.department && a.department_id.toString() === userDepartmentId
    ...
    departments.forEach((dept) => {
      ...
      if (
        (!user?.department_id ||
          dept.id.toString() !== user.department_id.toString()) &&
      ...
      <div className="flex justify-between p-4">
        <div>
          <h1 className="text-xl font-bold text-gray-800">
            HOD Agreement Approval
          </h1>
          <p className="text-sm text-gray-600 mt-1">
            Review and approve performance agreements pending your approval as
            Head of Department
          </p>
        </div>
      </div>
            ...
            <div className="flex">
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  ...
                  or Line Manager. You can review, approve, or reject these
                  agreements.
                </p>
              </div>
            </div>
                ...
                {filteredAgreements.length === 0 ? (
                  ...
                  filteredAgreements.map((agreement) => (
                    <TableRow key={agreement.id} className="hover:bg-gray-50">
                        ...
                        <div className="text-sm font-medium text-gray-900">
                          {agreement.creator
                            ? `${agreement.creator.surname} ${
                                agreement.creator.first_name
                              }${
                                agreement.creator.other_name
                                  ? " " + agreement.creator.other_name
                                  : ""
                              }`
                            : "Unknown"}
                      ...
                      <TableCell>
                        {agreement.creator?.unit_or_branch?.name ||
                          "Head Office"}
                      </TableCell>
                      <TableCell>
