...
const StatusBadge = ({ status }) => {
  const statusConfig = {
    pending_peer: { label: 'Pending Peer Approval', color: 'bg-amber-100 text-amber-700', icon: ClockIcon },
...
// Peer Approval Modal Component
const PeerApprovalModal = ({ isOpen, closeModal, appraisal, onApprove, onReject }) => {
  const [peerComments, setPeerComments] = useState('');

  const handleApprove = () => {
    if (onApprove) {
      onApprove(appraisal, peerComments);
    }
    closeModal();
  };

  const handleReject = () => {
    if (peerComments.trim() === '') {
      alert('Please provide comments explaining why you are rejecting this appraisal');
      return;
    }
    
    if (onReject) {
      onReject(appraisal, peerComments);
    }
    closeModal();
  };
              ...
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-xl bg-white text-left align-middle shadow-xl transition-all border border-teal-100">
                ...
                <div className="px-7 pt-7 pb-4 max-h-[80vh] overflow-y-auto">
                  <div className="space-y-6">
                    ...
                    <div>
                      <label htmlFor="peerComments" className="block text-sm font-medium text-gray-700 mb-1">
                        Peer Comments
                      </label>
                      <textarea
                        id="peerComments"
    ...
    const updatedAppraisals = appraisals.map(a => {
      if (a.id === appraisal.id) {
        return { 
          ...
          peerName: user?.name || 'Current Peer User' // In a real app, use the logged-in user's name
        };
      }
      return a;
    });
  ...
  return (
    <div className="min-h-screen bg-white shadow-md rounded-lg">
      ...
      <div className="flex justify-between p-4 bg-gray-100">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Peer Approval</h1>
          <p className="text-sm text-gray-600 mt-1">
            Review and approve performance ratings as a Peer Reviewer
          </p>
        </div>
          ...
          filters={[
            ...
            },
            {
              ...
              options: [
                ...
                { value: 'pending_peer', label: 'Pending Peer Approval' },
